...
#define BUFFER_SIZE 8
  ...
  2.3线程间的互斥
  互斥量和临界区类似，只有拥有互斥量的线程才具有访问资源的权限，由于互斥对象只有一个，这就决定了
  任何情况下共享资源（代码或变量）都不会被多个线程同时访问。使用互斥不仅能够在同一应用程序的不同线程中实现资源的安全共享，
  而且可以在不同应用程序的线程之间实现对资源的安全共享。Linux中通过pthread_mutex_t来定义互斥体机制完成互斥操作。具体的操作函数如下

  pthread_mutex_init(pthread_mutex_t *mutex,const pthread_mutexattr_t *attr);初使化一个互斥体变量mutex，
  参数attr表示按照attr属性创建互斥体变量mutex，如果参数attr为NULL，则以默认的方式创建。

  pthread_mutex_lock(pthread_mutex_t *mutex);给一个互斥体变量上锁，如果mutex指定的互斥体已经被锁住，
  则调用线程将被阻塞直到拥有mutex的线程对mutex解锁为止。

  pthread_mutex_unlock(pthread_mutex_t *mutex);对参数mutex指定的互斥体变量解锁。

  2.4线程间的同步
  同步就是线程等待某一个事件的发生，当等待的事件发生时，被等待的线程和事件一起继续执行。如果等待的事件未到达则挂起。
  在linux操作系统中是通过条件变量来实现同步的。
